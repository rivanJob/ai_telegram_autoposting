...
<p>Variables helper: <code>{{audience}}</code> <code>{{tone}}</code> <code>{{category}}</code> <code>{{season}}</code></p>
<form method="post"><input type="hidden" name="_csrf" value="<?= e(CsrfGuard::token()) ?>"><div class="row"><select name="theme_id"><?php foreach($themes as $t):?><option value="<?= (int)$t['id'] ?>"><?= e($t['name']) ?></option><?php endforeach;?></select><input name="title" placeholder="Prompt title" required></div><textarea name="body" rows="8" placeholder="Strict JSON-only prompt" required></textarea><button>Save New Version</button></form>
<table><tr><th>ID</th><th>Theme</th><th>Version</th><th>Title</th><th>Created</th></tr><?php foreach($rows as $r): ?><tr><td><?= (int)$r['id'] ?></td><td><?= e($r['theme']) ?></td><td><?= (int)$r['version'] ?></td><td><?= e($r['title']) ?></td><td><?= e($r['created_at']) ?></td></tr><?php endforeach; ?></table>
<?php renderLayout('Prompt Manager', ob_get_clean());
