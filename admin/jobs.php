...
ob_start(); ?>
<h1>Jobs Monitor</h1>
<form><select name="status"><option value="">All</option><option <?= $status==='NEW'?'selected':'' ?>>NEW</option><option <?= $status==='RUNNING'?'selected':'' ?>>RUNNING</option><option <?= $status==='DONE'?'selected':'' ?>>DONE</option><option <?= $status==='ERROR'?'selected':'' ?>>ERROR</option></select><button>Filter</button></form>
<table><tr><th>ID</th><th>Status</th><th>Channel</th><th>Theme</th><th>Run</th><th>Error</th><th>Actions</th></tr><?php foreach($rows as $r): ?><tr><td><?= (int)$r['id'] ?></td><td><?= e($r['status']) ?></td><td><?= e($r['channel']) ?></td><td><?= e($r['theme']) ?></td><td><?= e($r['run_at']) ?></td><td><?= e($r['last_error']) ?></td><td><details><summary>detail</summary><pre><?= e($r['llm_raw']) ?></pre><pre><?= e($r['telegram_raw']) ?></pre></details><form method="post"><input type="hidden" name="_csrf" value="<?= e(CsrfGuard::token()) ?>"><input type="hidden" name="job_id" value="<?= (int)$r['id'] ?>"><button name="action" value="retry">Retry</button></form></td></tr><?php endforeach;?></table>
<?php renderLayout('Jobs', ob_get_clean());
